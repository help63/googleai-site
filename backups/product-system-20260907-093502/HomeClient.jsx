...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/site-settings", { cache: "no-store" })
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
    ]).then(([contentData, garmentData, tvData, settings]) => {
      if (!alive) return;
      ...
      if (garmentData?.success) {
        setGarments(
          (garmentData.items || []).filter(
  ...
  return (
    <>
    <AdManager slot="2986048088940479" />
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ color: "#94a3b8" }}>
              Latest clothing products and prices. ({garments.length} products)
            </p>
          </div>
